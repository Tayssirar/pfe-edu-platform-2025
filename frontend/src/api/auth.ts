...
export const login = async (data: any) => {
  try {
    const response = await fetchWithRetry('post',`${API_URL}/auth/login`, data);
    return response;
  } catch (error) {
...
export const register = async (data: any) => {
  try {
    return await fetchWithRetry('post',`${API_URL}/auth/register`, data);
  } catch (error) {
...
export const updateAvatar = async (userId: string, role: string, avatar: Avatar) => {
  ...
  try {
    await fetchWithRetry('put',`${API_URL}/auth/${role}/${userId}/avatar`, { avatar }); // Ensure the payload is correct
  } catch (error) {
...
export const fetchUserInfo = async (userId: string, role: string) => {
  try {
    const response = await fetchWithRetry('get',`${API_URL}/auth/${role}/${userId}`);
